...
import com.example.cms.admin.external.service.AdminGasStationService;
 ...
 */
@Controller
@RequestMapping("/admin/external/gas-stations")
@RequiredArgsConstructor
public class AdminGasStationController {

    private final AdminGasStationService gasStationService;
    ...
    public String gasStationList(@RequestParam(value = "keyword", required = false) String keyword,
        ...
        model.addAttribute("content", "admin/external/gas-station-list");
    ...
    public String gasStationDetail(@PathVariable("id") Long id, Model model) {
        ...
        if (gasStation == null) {
            return "redirect:/admin/external/gas-stations";
        }

        model.addAttribute("gasStation", gasStation);
        model.addAttribute("title", "주유소 상세");
        model.addAttribute("content", "admin/external/gas-station-detail");
        return "admin/layout/admin-layout";
    }
