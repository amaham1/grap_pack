 ...
 * akapwhd 계정 비밀번호로 인증 후 히스토리/방문자 데이터를 단일 페이지에서 열람
 */
@Slf4j
@Controller
...
public class QrGenDataController {
    ...
    /**
     * 데이터 열람 페이지 (히스토리 + 방문자 통합)
     * tab 파라미터로 현재 탭 구분: history(기본), visitor
     */
    @GetMapping
    public String showQrGenData(@RequestParam(value = "tab", defaultValue = "history") String tab,
        ...
        if ("visitor".equals(tab)) {
            List<QrGenVisitor> visitorList = qrGenVisitorService.findAllQrGenVisitors(page, PAGE_SIZE);
            int totalCount = qrGenVisitorService.countAllQrGenVisitors();
    ...
    public String verifyQrGenDataPassword(@RequestParam("password") String password,
        ...
        if (user != null && passwordEncoder.matches(password, user.getQrGenUserPassword())) {
            ...
            log.info("✅ [CHECK] 데이터 열람 인증 성공");
            return "redirect:/qrgen/data";
        }

        log.warn("❌ [ERROR] 데이터 열람 인증 실패");
