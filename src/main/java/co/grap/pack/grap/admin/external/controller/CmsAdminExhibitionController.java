...
@Slf4j
@Controller
@RequestMapping("/grap/admin/external/exhibitions")
@RequiredArgsConstructor
public class CmsAdminExhibitionController {

    private final CmsAdminExhibitionService exhibitionService;
    ...
    public String exhibitionList(@RequestParam(value = "keyword", required = false) String keyword,
        ...
        model.addAttribute("content", "grap/admin/external/cms-exhibition-list");
    ...
    public String exhibitionDetail(@PathVariable("id") Long id, Model model) {
        ...
        if (exhibition == null) {
            return "redirect:/grap/admin/external/exhibitions";
        }

        model.addAttribute("exhibition", exhibition);
        model.addAttribute("title", "공연/전시 상세");
        model.addAttribute("content", "grap/admin/external/cms-exhibition-detail");
        return "grap/admin/layout/cms-admin-layout";
    }
    ...
    public String updateMemo(@PathVariable("id") Long id,
        ...
        return "redirect:/grap/admin/external/exhibitions/" + id;
    }
    ...
    public ResponseEntity<String> bulkUpdateIsShow(@RequestBody BulkUpdateRequest request) {
        try {
            log.info("✅ [CHECK] 공연/전시 노출여부 일괄 변경 요청: ids={}, isShow={}", request.getIds(), request.getIsShow());
            exhibitionService.bulkUpdateIsShow(request.getIds(), request.getIsShow());
            log.info("✅ [CHECK] 공연/전시 {}개 항목 노출여부 일괄 변경 완료", request.getIds().size());
            return ResponseEntity.ok("일괄 처리 완료");
        } catch (Exception e) {
            log.error("❌ [ERROR] 공연/전시 노출여부 일괄 변경 실패: {}", e.getMessage());
