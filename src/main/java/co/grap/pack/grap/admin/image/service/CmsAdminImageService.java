...
public class CmsAdminImageService {
     ...
     * 콘텐츠별 이미지 목록 조회
     */
    public List<CmsAdminImage> getImageList(Long contentId) {
     ...
     * 이미지 상세 조회
     */
    public CmsAdminImage getImage(Long imageId) {
    ...
    public void uploadImage(Long contentId, MultipartFile file, Integer displayOrder) throws IOException {
        ...
        // 이미지 정보 저장
        CmsAdminImage image = CmsAdminImage.builder()
                .contentId(contentId)
    ...
    public void deleteImage(Long imageId) throws IOException {
        // 이미지 정보 조회
        CmsAdminImage image = adminImageMapper.selectImageById(imageId);
        if (image == null) {
    ...
    public void deleteImagesByContentId(Long contentId) throws IOException {
        // 모든 이미지 조회
        List<CmsAdminImage> imageList = adminImageMapper.selectImageListByContentId(contentId);

        // 파일 삭제
        for (CmsAdminImage image : imageList) {
            fileUtil.deleteFile(image.getSavedName());
        }
