package co.grap.pack.grap.admin.sync.service;
...
public class CmsSyncManager {
    ...
    public void startSync(String sessionId) {
        log.info("===> [CmsSyncManager] 동기화 시작 - 세션ID: {}", sessionId);
        cancelFlags.put(sessionId, new AtomicBoolean(false));
        log.info("===> [CmsSyncManager] 현재 활성 세션 수: {}", cancelFlags.size());
    }
    ...
    public void cancelSync(String sessionId) {
        log.warn("===> [CmsSyncManager] *** 동기화 중단 요청 *** - 세션ID: {}", sessionId);
        AtomicBoolean flag = cancelFlags.get(sessionId);
        if (flag != null) {
            boolean oldValue = flag.getAndSet(true);
            log.warn("===> [CmsSyncManager] 중단 플래그 설정: {} -> true", oldValue);
        } else {
            log.warn("===> [CmsSyncManager] 경고: 세션ID {}에 대한 플래그를 찾을 수 없음", sessionId);
            log.warn("===> [CmsSyncManager] 현재 활성 세션 목록: {}", cancelFlags.keySet());
        }
    }
    ...
    public void completeSync(String sessionId) {
        log.info("===> [CmsSyncManager] 동기화 완료 - 세션ID: {}", sessionId);
        cancelFlags.remove(sessionId);
        log.info("===> [CmsSyncManager] 남은 활성 세션 수: {}", cancelFlags.size());
    }
    ...
    public boolean isCancelled(String sessionId) {
        ...
        if (cancelled) {
            log.warn("===> [CmsSyncManager] 중단 플래그 감지! - 세션ID: {}", sessionId);
        }
        return cancelled;
    }
    ...
    public void checkCancellation(String sessionId) {
        log.debug("===> [CmsSyncManager] 중단 체크 - 세션ID: {}", sessionId);
        if (isCancelled(sessionId)) {
            log.error("===> [CmsSyncManager] !!! 동기화 중단 예외 발생 !!! - 세션ID: {}", sessionId);
