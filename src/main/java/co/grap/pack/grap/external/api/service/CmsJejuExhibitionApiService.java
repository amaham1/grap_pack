...
import co.grap.pack.grap.external.api.mapper.CmsExternalExhibitionMapper;
...
public class CmsJejuExhibitionApiService {
    ...
    private final CmsExternalExhibitionMapper exhibitionMapper;
     ...
     */
    public List<Map<String, Object>> fetchAllExhibitionDataFromApi(String sessionId, CmsSyncManager syncManager) {
     ...
     */
    public List<CmsExternalExhibition> transformApiDataToEntity(List<Map<String, Object>> items) {
        List<CmsExternalExhibition> exhibitions = new ArrayList<>();
        LocalDateTime fetchedAt = LocalDateTime.now();

        for (Map<String, Object> item : items) {
            try {
                CmsExternalExhibition exhibition = CmsExternalExhibition.builder()
     ...
     */
    @Transactional
    public int saveExhibitions(List<CmsExternalExhibition> exhibitions) {
        int count = 0;

        for (CmsExternalExhibition exhibition : exhibitions) {
            try {
     ...
     */
    @Transactional
    public void syncExhibitionsFromExternalApi(String sessionId, CmsSyncManager syncManager) {
        try {
            ...
            // 2. Entity로 변환
            List<CmsExternalExhibition> exhibitions = transformApiDataToEntity(items);
