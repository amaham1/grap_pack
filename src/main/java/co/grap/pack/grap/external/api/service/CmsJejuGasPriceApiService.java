...
import co.grap.pack.grap.external.api.mapper.CmsExternalGasPriceMapper;
import co.grap.pack.grap.external.api.mapper.CmsExternalGasStationMapper;
...
public class CmsJejuGasPriceApiService {
    ...
    private final CmsExternalGasStationMapper gasStationMapper;
    private final CmsExternalGasPriceMapper gasPriceMapper;
     ...
     */
    @SuppressWarnings("unchecked")
    public List<CmsExternalGasStation> transformApiDataToStationEntity(Map<String, Object> apiResponse) {
        List<CmsExternalGasStation> stations = new ArrayList<>();
        ...
        try {
            ...
            for (Map<String, Object> item : info) {
                try {
                    ...
                    CmsExternalGasStation station = CmsExternalGasStation.builder()
     ...
     */
    @SuppressWarnings("unchecked")
    public List<CmsExternalGasPrice> transformApiDataToPriceEntity(Map<String, Object> apiResponse) {
        List<CmsExternalGasPrice> prices = new ArrayList<>();
        ...
        try {
            ...
            for (Map<String, Object> item : info) {
                try {
                    ...
                    CmsExternalGasPrice price = CmsExternalGasPrice.builder()
                            .opinetId(opinetId.trim())
     ...
     */
    @Transactional
    public int saveGasStations(List<CmsExternalGasStation> stations) {
        int count = 0;

        for (CmsExternalGasStation station : stations) {
            try {
     ...
     */
    @Transactional
    public int saveGasPrices(List<CmsExternalGasPrice> prices) {
        int count = 0;
        int skipped = 0;

        for (CmsExternalGasPrice price : prices) {
            try {
                // FK 검증: 주유소가 존재하는지 확인
     ...
     */
    @Transactional
    public void syncGasPricesFromExternalApi(String sessionId, CmsSyncManager syncManager) {
        try {
            ...
            List<CmsExternalGasStation> stations = transformApiDataToStationEntity(apiResponse);
            ...
            List<CmsExternalGasPrice> prices = transformApiDataToPriceEntity(apiResponse);
