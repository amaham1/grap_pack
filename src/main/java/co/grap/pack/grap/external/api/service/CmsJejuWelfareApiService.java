...
import co.grap.pack.grap.external.api.mapper.CmsExternalWelfareServiceMapper;
...
public class CmsJejuWelfareApiService {
    ...
    private final CmsExternalWelfareServiceMapper welfareServiceMapper;
     ...
     */
    @SuppressWarnings("unchecked")
    public List<CmsExternalWelfareService> transformApiDataToEntity(Map<String, Object> apiResponse) {
        List<CmsExternalWelfareService> services = new ArrayList<>();
        ...
        try {
            ...
            for (Map<String, Object> item : list) {
                try {
                    CmsExternalWelfareService service = CmsExternalWelfareService.builder()
     ...
     */
    @Transactional
    public int saveWelfareServices(List<CmsExternalWelfareService> services) {
        int count = 0;

        for (CmsExternalWelfareService service : services) {
            try {
     ...
     */
    @Transactional
    public void syncWelfareServicesFromExternalApi(String sessionId, CmsSyncManager syncManager) {
        try {
            ...
            // 2. Entity로 변환
            List<CmsExternalWelfareService> services = transformApiDataToEntity(apiResponse);
